...
function Review() {
  ...
  const { array}=useSelector((state)=>state.moreDetails)
    ...
    const getStarted=()=>{
      setIsloading(true)
      axios.post("/api/update",{value:array,token:cookies.userToken,name:"jobDetails"}).then((res)=>{
        if(res.data.status){
          router.push("/cv");
          dispatch(Reset())
          dispatch(userReset())
                          ...
                          <div className="col-sm-12 col-md-12 col-lg-6">
                            <div className="formSide p-5">
                              ...
                              <hr/>
                              {
                                array.map((val,index)=>(
                                  <>
                                   <span className="fw-bold text-dark">{index+1}.<span className="fw-bold text-secondary">{val.jobTitle},{val.employer}</span></span> <br/>
                            <small className="mx-3">{val.startYear}</small> to <small className="mx-3">{val.endYear}</small><br/>

                                  </>
                                ))
                              }
                           
                            <button className="addPosition" onClick={addPosition}>+ Add another Position </button>
